...
class OperationDialog(GObject.GObject):
	...
	# how many points to aggregate
	MAX_SPEED_POINTS = 20
	
	# GObject name type
	__gtype_name__ = 'Sunflower_OperationDialog'  
	...
	def __init__(self, application, thread):
		...
		self.window.set_title('Operation Dialog')
		self.window.set_default_size(500, 10)
		...
		self.window.connect('delete-event', self._cancel_click)
		self.window.connect('window-state-event', self._window_state)
	...
	def _confirm_cancel(self, message):
		...
		return result is Gtk.ResponseType.YES

	def _minimize_click(self, widget, data=None):
		"""Handle minimize click"""
		self.window.iconify()

		# support for window managers without iconify support 
	...
	def _window_state(self, widget, event, data=None):
		...
		if event.new_window_state is Gdk.WindowState.ICONIFIED:
...
class CopyDialog(OperationDialog):
	...
	def __init__(self, application, thread):
		...
		# configure layout
		self.window.set_title(_('Copy Selection'))
	...
	def _set_operation_image(self, icon_name=None):
		...
		if icon_name is None:
			self._operation_image.set_from_icon_name('folder-copy', Gtk.IconSize.MENU)
...
class MoveDialog(CopyDialog):
	...
	def __init__(self, application, thread):
		...
		# configure layout
		self.window.set_title(_('Move Selection'))
	...
	def _set_operation_image(self, icon_name=None):
		...
		# set default icon
		if icon_name is None:
			self._operation_image.set_from_icon_name('folder-move', Gtk.IconSize.MENU)
...
class DeleteDialog(OperationDialog):
	...
	def __init__(self, application, thread):
		...
		# configure layout
		self.window.set_title(_('Delete Selection'))
